...
export function findPath(
  ...
  let minDistance = Math.abs(startCol - endCol) + Math.abs(startRow - endRow)
  ...
  while (queue.length > 0) {
    ...
    if (currKey === endKey) {
      // Reconstruct path to exact target
      closestNodeKey = endKey
      break
    }

    // Update closest if we found a strictly better node
    const dist = Math.abs(curr.col - endCol) + Math.abs(curr.row - endRow)
    if (dist < minDistance) {
      minDistance = dist
      closestNodeKey = currKey
    }
    ...
    for (const d of dirs) {
      ...
      if (!isWalkable(nc, nr, tileMap, blockedTiles)) {
        // Still mark visited so we don't re-check walls unnecessarily
        visited.add(nk)
        continue
      }
  ...
  // Reconstruct path to the best node found (closestNodeKey)
  if (closestNodeKey === startKey) return []

  const path: Array<{ col: number; row: number }> = []
  let k = closestNodeKey
  while (k !== startKey) {
